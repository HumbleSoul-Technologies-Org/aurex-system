...
export async function createTenantInvite(
  ...
  try {
    ...
    const authToken =
      token ||
      (typeof window !== "undefined" ? tokenManager.getAuthToken() : null);
...
export async function validateTenantInvite(token: string) {
  try {
...
export async function acceptTenantInvite(token: string): Promise<boolean> {
  try {
    ...
    console.warn(
      "acceptTenantInvite API failed, falling back to localStorage:",
      err?.message,
    );
    const invites = listTenantInvitesLocal();
...
export function getTenantInviteByToken(
  token: string,
): TenantInviteRecord | null {
  return (
    listTenantInvitesLocal().find((invite) => invite.token === token) || null
  );
}
...
export function updateTenantInvite(
  id: string,
  updates: Partial<TenantInviteRecord>,
): boolean {
  const invites = listTenantInvitesLocal();
  const index = invites.findIndex((invite) => invite.id === id);
  if (index === -1) return false;
