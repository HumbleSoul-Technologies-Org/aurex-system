...
const DEFAULT_TOKEN =
  ADMIN_TOKEN || PROPERTY_MANAGER_TOKEN || TENANT_TOKEN || "";
...
const tests = [
  ...
  {
    name: "Tenant outstanding balance /payments/tenant/:tenantId/balance",
    path: () => `/payments/tenant/${TENANT_ID}/balance`,
    token: TENANT_TOKEN,
    expected: [200, 401, 403, 404],
    requires: [TENANT_ID, TENANT_TOKEN],
    skipIfMissing: "TENANT_ID and TENANT_TOKEN",
  ...
  },
  {
    ...
    skipIfMissing:
      "SETTINGS_ID and ADMIN_TOKEN/PROPERTY_MANAGER_TOKEN/TENANT_TOKEN",
  },
  {
...
async function run() {
  ...
  for (const test of tests) {
    ...
    if (requires.length > 0 && !requires.every(isPresent)) {
      console.log(
        `SKIP: ${test.name} -> missing ${test.skipIfMissing || "required values"}`,
      );
