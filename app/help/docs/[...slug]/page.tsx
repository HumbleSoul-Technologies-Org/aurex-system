...
export default async function DocPage({
  ...
  const name = slug.join("/") || "USER_GUIDE";
  const docsRoot = path.join(process.cwd(), "docs");
  const candidates = [
    path.join(docsRoot, `${name}.md`),
    path.join(docsRoot, "help", `${name}.md`),
    path.join(docsRoot, `${name.toUpperCase()}.md`),
  ];
    ...
    return (
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        <div className="lg:col-span-3">
          <article className="bg-card rounded-xl border border-border shadow-sm p-8 md:p-10 text-center">
            <div className="text-6xl mb-4">📄</div>
            <h1 className="text-2xl font-bold text-foreground mb-2">
              Documentation Not Found
            </h1>
            <p className="text-muted-foreground">
              We couldn't find the documentation page for:{" "}
            ...
            <a
              href="/help"
              className="inline-block mt-6 px-6 py-2 bg-primary hover:bg-primary/90 text-primary-foreground rounded-lg font-medium transition-colors"
            >
