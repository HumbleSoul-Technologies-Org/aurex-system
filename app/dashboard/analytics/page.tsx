    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground">Track your portfolio performance in real-time</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
        {[
          ...
          <Card key={metric.label} className="border border-border p-3 md:p-4">
            ...
            <p className="text-xl md:text-2xl font-bold text-foreground mt-1">{metric.value}</p>
            <div className={`flex items-center gap-1 mt-2 text-xs ${metric.color}`}>
              {metric.trend === 'up' ? (
      ...
      <Card className="border border-border p-4 md:p-6">
        <h3 className="text-lg font-bold text-foreground mb-4">Property Performance</h3>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {[
                { name: 'Sunset Apartments', revenue: '$18,500', occupancy: '95%', roi: '22%', status: 'Excellent' },
                { name: 'Downtown Office', revenue: '$15,200', occupancy: '88%', roi: '18%', status: 'Good' },
                { name: 'Beachside Villa', revenue: '$12,800', occupancy: '100%', roi: '25%', status: 'Excellent' },
                { name: 'Mountain Lodge', revenue: '$11,500', occupancy: '85%', roi: '16%', status: 'Good' },
                { name: 'Urban Lofts', revenue: '$9,800', occupancy: '92%', roi: '20%', status: 'Good' },
              ].map((property, idx) => (
                <tr key={idx} className="border-b border-border hover:bg-secondary">
                  <td className="py-3 px-2 text-foreground font-medium">{property.name}</td>
                  <td className="py-3 px-2 text-foreground hidden sm:table-cell">{property.revenue}</td>
                  <td className="py-3 px-2 text-foreground hidden md:table-cell">{property.occupancy}</td>
                  <td className="py-3 px-2 text-foreground hidden lg:table-cell">{property.roi}</td>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card className="border border-border p-4 md:p-6">
          <h3 className="text-lg font-bold text-foreground mb-4">Top Performing Property</h3>
          <div className="space-y-3">
            <div className="flex items-start justify-between">
              <div>
                <p className="font-medium text-foreground">Beachside Villa</p>
                <p className="text-sm text-muted-foreground">$12,800 monthly revenue</p>
              </div>
        ...
        <Card className="border border-border p-4 md:p-6">
          <h3 className="text-lg font-bold text-foreground mb-4">Areas for Improvement</h3>
          <div className="space-y-3">
            <div className="flex items-start justify-between">
              <div>
                <p className="font-medium text-foreground">Mountain Lodge</p>
                <p className="text-sm text-muted-foreground">$11,500 monthly revenue</p>
              </div>
            ...
            <p className="text-xs text-muted-foreground">85% occupancy - 3 units vacant</p>
          </div>
        </Card>
      </div>
    </div>
