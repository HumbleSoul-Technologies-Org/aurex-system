...
import { getMaintenanceRequests, updateMaintenanceRequest, deleteMaintenanceRequest, type MaintenanceRequest } from '@/lib/services/maintenance'
...
export default function MaintenancePage() {
  ...
  const enrichedRequests: MaintenanceRequest[] = useMemo(() => rawRequests.map((req) => {
    const tenant = req.tenantId ? allTenants.find((t) => t.id === req.tenantId) : null
    const property = req.propertyId ? getProperty(req.propertyId) : null

    return {
      id: req.id,
      propertyId: req.propertyId,
      propertyName: property?.name || req.propertyName || 'Unknown Property',
      unit: req.unit,
  ...
  const [expenseForm, setExpenseForm] = useState({ maintenanceId: '', expense: '', expenseType: '', description: '', tenantId: '', price: '', paymentMethod: '', companyAssigned: '', propertyId: '', unit: '', date: '', receiptReference: '' })

  // Get the selected property for conditional unit display
  const selectedProperty = useMemo(() => {
    return expenseForm.propertyId ? allProperties.find(p => p.id === expenseForm.propertyId) : null
  ...
  const handleExpenseChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    if (name === 'propertyId') {
      ...
      if (property && property.units.length === 1) {
        ...
        setExpenseForm((prev) => ({ ...prev, propertyId: value, unit: property.units[0] }))
      } else {
  ...
  const handleExpenseSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      setExpenseForm({ maintenanceId: '', expense: '', expenseType: '', description: '', tenantId: '', price: '', paymentMethod: '', companyAssigned: '', propertyId: '', unit: '', date: '', receiptReference: '' })
      ...
      <div className="space-y-3 pr-10">
        <div className="flex items-start justify-between gap-2">
          <div className="flex-1">
            <h3 className="font-semibold text-foreground text-sm line-clamp-2">{request.category}</h3>
        ...
        <div className="pt-2 border-t border-border space-y-2">
          ...
          <div className="flex items-center gap-2 text-xs">
            ...
            <span className="text-muted-foreground">Requested on: {new Date(request.createdDate).toLocaleDateString()} </span>
  ...
  return (
    <div className="space-y-6">
        ...
        <Card className="border border-border p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-foreground">{enrichedRequests.length}</p>
            </div>
        ...
        <Card className="border border-border p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-foreground">${enrichedRequests.reduce((sum, r) => sum + (r.cost || 0), 0).toLocaleString()}</p>
            </div>
              ...
              <tbody>
                {enrichedRequests.map((request) => (
                  <tr key={request.id} className="border-b border-border hover:bg-secondary transition-colors">
                    <td className="px-6 py-4 font-semibold text-foreground text-sm">{request.category}</td>
                    <td className="px-6 py-4   text-foreground text-sm">{request.description}</td>
                    <td className="px-6 py-4 text-muted-foreground text-sm">{request.propertyName}</td>
                    <td className="px-6 py-4 text-muted-foreground text-sm">{request.unit}</td>
      ...
      {/* Expense Dialog */}
      <Dialog open={expenseDialogOpen} onOpenChange={(open) => { if(!open){ setSelectedRequestId(null); setExpenseForm({ maintenanceId: '', expense: '', expenseType: '', description: '', tenantId: '', price: '', paymentMethod: '', companyAssigned: '', property: '', unit: '', date: '', receiptReference: '' }) } setExpenseDialogOpen(open) }}>
        <DialogContent>
          <DialogHeader>
          ...
          <form onSubmit={(e) => { handleExpenseSubmit(e) }} className="p-2">
            <div className="space-y-4 py-2">
              <div>
                <label className="block text-sm font-medium text-foreground mb-2">Expense Category</label>
                <select name="expenseType" value={expenseForm.expenseType} onChange={handleExpenseChange} className="w-full px-3 py-2 border border-input rounded-md bg-background text-foreground" disabled={isSavingExpense}>
              ...
              <div>
                ...
                <Textarea name="description" value={expenseForm.description} onChange={handleExpenseChange} placeholder="Details about the expense" className="h-24" disabled={isSavingExpense} />
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-foreground mb-2">Date</label>
                  <Input type="date" name="date" value={expenseForm.date} onChange={handleExpenseChange} disabled={isSavingExpense} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-foreground mb-2">Receipt/Invoice Reference</label>
                  <Input name="receiptReference" value={expenseForm.receiptReference} onChange={handleExpenseChange} placeholder="Reference number" disabled={isSavingExpense} />
                </div>
              </div>

              <div>
                <label className="block text-sm font-medium text-foreground mb-2">Tenant</label>
                <select name="tenantId" value={expenseForm.tenantId} onChange={handleExpenseChange} className="w-full px-3 py-2 border border-input rounded-md bg-background text-foreground" disabled={isSavingExpense}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <select name="propertyId" value={expenseForm.propertyId} onChange={handleExpenseChange} className="w-full px-3 py-2 border border-input rounded-md bg-background text-foreground" disabled={isSavingExpense}>
                ...
                {selectedProperty && selectedProperty.units.length > 1 && (
                  <div>
                    ...
                    <select name="unit" value={expenseForm.unit} onChange={handleExpenseChange} className="w-full px-3 py-2 border border-input rounded-md bg-background text-foreground" disabled={isSavingExpense}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <Input type="text" name="price" value={expenseForm.price} onChange={handleExpenseChange} placeholder="0" disabled={isSavingExpense} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-foreground mb-2">Company Assigned</label>
                  <Input name="companyAssigned" value={expenseForm.companyAssigned} onChange={handleExpenseChange} placeholder="Company name" disabled={isSavingExpense} />
                </div>
              </div>

              <div>
                <label className="block text-sm font-medium text-foreground mb-2">Payment Method</label>
                <select name="paymentMethod" value={expenseForm.paymentMethod} onChange={handleExpenseChange} className="w-full px-3 py-2 border border-input rounded-md bg-background text-foreground" disabled={isSavingExpense}>
            ...
            <DialogFooter>
              <div className="flex gap-2">
                <Button 
                  type="button" 
                  variant="outline" 
                  onClick={() => { setExpenseDialogOpen(false); setSelectedRequestId(null); setExpenseForm({ maintenanceId: '', expense: '', expenseType: '', description: '', tenantId: '', price: '', paymentMethod: '', companyAssigned: '', propertyId: '', unit: '', date: '', receiptReference: '' }) }}
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={confirmDeleteRequest} disabled={deletingId !== null}>
