...
export default function TenantPortalPage() {
  ...
  const tenantInvitations = useMemo(() => {
    return tenants.slice(0, 5).map((tenant, idx) => {
      ...
      const dates = ['2024-02-05', '2024-02-03', '2024-02-01', '2024-01-28', '2024-01-25']
      return {
        id: tenant.id,
        tenant: tenant.name,
        email: tenant.email,
  ...
  const handleTenantAction = (tenant: TenantRecord & { propertyName: string; lastActive: string; portalStatus: string }, action: string) => {
  ...
  const handleConfirmAction = () => {
    switch (dialogAction) {
      case 'ban':
        ...
        alert(`Tenant ${selectedTenant?.name} has been banned from the portal.`)
        break
      case 'message':
  ...
  useEffect(() => {
    ...
    if (savedSettings) {
      try {
        ...
        setPortalFeatures(prev => prev.map(feature => ({
          ...feature,
  ...
  const handleSaveSettings = () => {
    ...
    const settings = portalFeatures.reduce((acc, feature) => {
  ...
  return (
    <div className="space-y-6">
              ...
              {enrichedTenants.length === 0 ? (
                ...
                enrichedTenants.map((tenant) => (
                  <tr key={tenant.id} className="border-b border-border hover:bg-secondary">
                    ...
                    <td className="py-3 px-2 text-muted-foreground hidden sm:table-cell">{tenant.propertyName}</td>
                    ...
                    <td className="py-3 px-2 text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem onClick={() => handleTenantAction(tenant, 'message')}>
                            <Mail className="mr-2 h-4 w-4" />
                            Message
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => handleTenantAction(tenant, 'profile')}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Tenants', value: tenantStats.totalTenants, icon: Users },
          { label: 'Active Portals', value: tenantStats.activePortals, icon: CheckCircle },
          { label: 'Pending Access', value: tenantStats.pendingAccess, icon: Clock },
          { label: 'Payment Success Rate', value: tenantStats.paymentSuccessRate, icon: DollarSign },
        ].map((stat) => {
      ...
      <Dialog open={tenantDialogOpen} onOpenChange={setTenantDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {dialogAction === 'ban' && `Are you sure you want to ban ${selectedTenant?.name} from accessing the tenant portal? This action cannot be undone.`}
              {dialogAction === 'message' && `Send a message to ${selectedTenant?.name}.`}
              {dialogAction === 'profile' && `View detailed profile information for ${selectedTenant?.name}.`}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            ...
            <Button
              ...
              className={dialogAction === 'ban' ? 'bg-red-600 hover:bg-red-700' : 'bg-primary hover:bg-primary/90'}
            >
