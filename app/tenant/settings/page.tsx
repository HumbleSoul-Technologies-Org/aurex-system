...
type NotificationType = 'overdue' | 'leaseEnd' | 'maintenance' | 'profileChanges' | 'messages'
...
export default function TenantSettingsPage() {
  ...
  const handlePasswordSave = () => {
    ...
    if (passwordState.currentPassword !== tenant.password) {
      setPasswordState((prev) => ({ ...prev, error: 'Current password is incorrect.' }))
      return
    }
    if (!passwordState.newPassword || passwordState.newPassword.length < 6) {
      setPasswordState((prev) => ({ ...prev, error: 'New password must be at least 6 characters.' }))
      return
    }
  ...
  return (
    <div className="space-y-6 md:space-y-8 max-w-6xl">
      <div>
        <h1 className="text-3xl md:text-4xl font-bold text-foreground mb-2">Tenant Settings</h1>
        <p className="text-sm md:text-base text-muted-foreground max-w-2xl">
          Configure your profile, notification preferences, move-out notice, emergency contact details, password, and document delivery options.
        </p>
      </div>
            ...
            <Card className="border border-border p-4 md:p-6">
              <div className="flex items-start gap-4 md:gap-6 mb-6 pb-6 border-b border-border">
                ...
                <div className="flex-1">
                  <h2 className="text-xl md:text-2xl font-bold text-foreground mb-1">Profile</h2>
          ...
          {activeTab === 'notifications' && (
            <Card className="border border-border p-4 md:p-6">
              <div className="flex items-start gap-4 mb-6 pb-6 border-b border-border">
                ...
                <div>
                  <h2 className="text-xl md:text-2xl font-bold text-foreground mb-1">Notification Preferences</h2>
                  <p className="text-sm text-muted-foreground">Control which notifications you receive and how you receive them.</p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                {[
                  ...
                  },
                  {
                    ...
                    description: 'Notify me when a maintenance visit is scheduled',
                  },
                  {
                  ...
                  },
                  {
                    ...
                    description: 'Notify me of new messages when I am logged out',
                  },
                ].map((item) => (
                  <div key={item.key} className="rounded-2xl border border-border p-4 bg-background">
              ...
              <div className="pt-6 border-t border-border mt-6">
                <Button
                  onClick={() => handleSave('notifications', { notificationPreferences })}
                  className="bg-primary hover:bg-primary/90 text-white gap-2"
                >
          ...
          {activeTab === 'emergency' && (
            <Card className="border border-border p-4 md:p-6">
              <div className="flex items-start gap-4 mb-6 pb-6 border-b border-border">
                ...
                <div>
                  <h2 className="text-xl md:text-2xl font-bold text-foreground mb-1">Emergency Contact</h2>
                  <p className="text-sm text-muted-foreground">Add someone we can reach if there is an urgent issue at your unit.</p>
                </div>
              </div>
          ...
          {activeTab === 'moveout' && (
            <Card className="border border-border p-4 md:p-6">
              <div className="flex items-start gap-4 mb-6 pb-6 border-b border-border">
                ...
                <div>
                  <h2 className="text-xl md:text-2xl font-bold text-foreground mb-1">Move-Out Notice</h2>
                  <p className="text-sm text-muted-foreground">Submit your planned move-out notice so your landlord can prepare for your departure.</p>
                </div>
              </div>

              {moveOutNotice.status === 'submitted' ? (
                <div className="space-y-4 rounded-2xl border border-primary/20 bg-primary/5 p-4">
                  <p className="font-semibold text-foreground">Notice Submitted</p>
                  <p className="text-sm text-muted-foreground">Your move-out notice has been submitted and cannot be edited.</p>
                  <div className="grid gap-3 sm:grid-cols-2">
                    <div>
                      <p className="text-xs text-muted-foreground">Notice Date</p>
                      <p className="font-medium text-foreground">{moveOutNotice.noticeDate || '—'}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground">Reason</p>
                      <p className="font-medium text-foreground">{moveOutReasons.find((item) => item.value === moveOutNotice.reason)?.label || '—'}</p>
                    </div>
                    <div className="sm:col-span-2">
                      <p className="text-xs text-muted-foreground">Forwarding Address</p>
                      <p className="font-medium text-foreground">{moveOutNotice.forwardingAddress || '—'}</p>
                    </div>
                    <div className="sm:col-span-2">
                      <p className="text-xs text-muted-foreground">Additional Notes</p>
                      <p className="font-medium text-foreground">{moveOutNotice.additionalNotes || '—'}</p>
                    </div>
                  </div>
                </div>
              ) : (
                <div className="space-y-6">
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    <div>
                      <label className="text-sm font-medium text-foreground mb-2 block">Notice Date</label>
                      <input
                        type="date"
                        value={moveOutNotice.noticeDate}
                        onChange={(e) => setMoveOutNotice({ ...moveOutNotice, noticeDate: e.target.value })}
                        className="w-full px-4 py-3 border border-border rounded-lg bg-background text-foreground text-sm"
                      />
                    </div>
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => setMoveOutNotice({ ...moveOutNotice, reason: e.target.value })}
                        className="w-full px-4 py-3 border border-border rounded-lg bg-background text-foreground text-sm"
                      >
                  ...
                  <div>
                    <label className="text-sm font-medium text-foreground mb-2 block">Forwarding Address</label>
                    <textarea
                      value={moveOutNotice.forwardingAddress}
                      onChange={(e) => setMoveOutNotice({ ...moveOutNotice, forwardingAddress: e.target.value })}
                      rows={4}
                      className="w-full px-4 py-3 border border-border rounded-lg bg-background text-foreground text-sm"
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) => setMoveOutNotice({ ...moveOutNotice, additionalNotes: e.target.value })}
                      rows={3}
                      className="w-full px-4 py-3 border border-border rounded-lg bg-background text-foreground text-sm"
                  ...
                  <div className="rounded-2xl border border-yellow-200 bg-yellow-50 p-4 text-sm text-foreground">
                    Sending this notice will notify your landlord of your planned move-out date. After submission, it will become read-only.
                  </div>

                  <div className="pt-6 border-t border-border mt-6">
                    <Button
                      onClick={() => {
                        setMoveOutNotice((prev) => ({ ...prev, status: 'submitted' }))
                        handleSave('moveout', {
                          moveOutNotice: { ...moveOutNotice, status: 'submitted' },
                        })
                      }}
                      className="bg-primary hover:bg-primary/90 text-white gap-2"
                    >
          ...
          {activeTab === 'security' && (
            <Card className="border border-border p-4 md:p-6">
              <div className="flex items-start gap-4 mb-6 pb-6 border-b border-border">
                ...
                <div>
                  <h2 className="text-xl md:text-2xl font-bold text-foreground mb-1">Security</h2>
                  <p className="text-sm text-muted-foreground">Change your password and keep your account secure.</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setPasswordState({ ...passwordState, currentPassword: e.target.value, error: '' })}
                    className="w-full px-4 py-3 border border-border rounded-lg bg-background text-foreground text-sm"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setPasswordState({ ...passwordState, newPassword: e.target.value, error: '' })}
                    className="w-full px-4 py-3 border border-border rounded-lg bg-background text-foreground text-sm"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setPasswordState({ ...passwordState, confirmPassword: e.target.value, error: '' })}
                    className="w-full px-4 py-3 border border-border rounded-lg bg-background text-foreground text-sm"
                  />
                </div>
              </div>
          ...
          {activeTab === 'documents' && (
            <Card className="border border-border p-4 md:p-6">
              <div className="flex items-start gap-4 mb-6 pb-6 border-b border-border">
                ...
                <div>
                  <h2 className="text-xl md:text-2xl font-bold text-foreground mb-1">Document Delivery</h2>
                  <p className="text-sm text-muted-foreground">Choose how you receive lease agreements, invoices, and notifications.</p>
                </div>
              </div>
                ...
                {documentDeliveryOptions.map((option) => (
                  ...
                  >
                    <input
                      ...
                      onChange={() => setDocumentDelivery(option.value as TenantRecord['documentDelivery'])}
